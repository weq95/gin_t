...
func (v Value) Bool(def ...bool) (bool, error) {
	...
	var i, err = strconv.ParseBool(string(v))
...
func (v Value) Duration(def ...int64) (time.Duration, error) {
	var i, err = v.Int64(def...)

	return time.Duration(i), err
}

func (v Value) Unmarshal(f Unmarshaler, dst any) error {
...
type warehouse map[any]any

func (w warehouse) Set(key, val any) {
	w[key] = val
}

func (w warehouse) Get(key any) (any, bool) {
	var val, ok = w[key]
